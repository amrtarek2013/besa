...
if (!class_exists('sql_db')) {
    ...
    class sql_db {
        ...
        function sql_nextid($seq_name = "") {
            $query_id = $this->query_result;

            if ($query_id && $this->last_query_text[$query_id] != "") {
                $this->last_query_text[$query_id] = str_replace("\"", "", $this->last_query_text[$query_id]);
                if (preg_match("/^INSERT[\t\n ]+INTO[\t\n ]+([a-z0-9\_\-]+)/is", $this->last_query_text[$query_id], $tablename)) {
                    if ($seq_name == "")
                        $seq_name = $tablename[1] . "_id_seq";
                    $query = "SELECT currval('" . $seq_name . "') AS last_value";
        ...
        function sql_error_msg($query_id = 0) {
            $result["message"] = @pg_errormessage($this->db_connect_id);

            return $result["message"];
        }

        function sql_error_code($query_id = 0) {
            $result['code'] = -1;

            return $result["code"];
        }

        function get_array($table_name, $fields = false, $filter = false, $order_by = false, $page = 1, $per_page = 10000, &$unlimited_count = 0) {
            if (is_array($fields))
            ...
            $sql = 'SELECT ' . $fields . ' FROM ' . $table_name . ' ' . $filter . ' ' . $order_by . ' LIMIT ' . ($page - 1) * $per_page . ", $per_page ";
            $rows = $this->sql_fetchrowset($this->sql_query($sql));

            $count_sql = 'SELECT COUNT(*) FROM ' . $table_name . ' ' . $filter;
            $result = $this->sql_query($sql);
            $row = $this->sql_fetchrow($result);
            $unlimited_count = $row[0];
...
global $db;
$db = new sql_db($site["dbhost"], $site["dblogin"], $site["dbpass"], $site["dbname"], false);
?>