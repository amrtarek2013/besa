 ...
 */
 ...
 /**
 *  IAM_CSVDump A class form performing a query dump and sending it to the browser or setting it or download.
 *  @author     Iván Ariel Melgrati <phpclasses@imelgrat.mailshell.com> - Postgre DB Handling by Umar Sear
 *  @package    iam_csvdump
 *  @version 1.0
 *
 *  IAM_CSVDump A class form performing a query dump and sending it to the browser or setting it or download.
...
class iam_csvdump
{
    ...
    */
    ...
    function GetFieldsNames($table_name, $dbname="mysql", $user="root", $password="", $host="localhost")
    {
      ...
      {
        $result = @pg_exec($conn,"SELECT * FROM $table_name ");
        if(!$result)
            die("Connected Successfully To The Database '$dbname' , Error. Cannot Select Fields From The Table '$table_name' ,Please Check The Table Name ");
        else
        {
            $fields = array();
            for($i=0;$i < (pg_num_fields($result));$i++)
            	{$fields[] = pg_field_name($result,$i); /*echo $fields[$i]."<br/>";*/}
           return $fields;
        }
      }
    }
    ...
    */
    ...
    function dump($query_string, $filename="dump", $ext="csv", $dbname="mysql", $user="root", $password="", $host="localhost", $list_fields=true,$seprator=";",$first_row=array() )
    {
            ...
            if ($filename!="")
            {
            	$CSV =$this->_generate_csv($query_string, $dbname, $user, $password, $host, $list_fields,$seprator);
                if ($CSV!="###ERROR###")
                {
