...
class f4wd
{
	...
	public function SetValues($_fwd_id , $_fwd_fname , $_fwd_lname , $_desc , $_suburb , $_tel , $_email , $_tag5 , $_tag6 , $_added_date , $_dont_send_mail , $_dont_send_sms , $_yard , $_home_phone , $_postcode , $_tag4)
	{
		$this->fwd_fname=$_fwd_fname;
		$this->fwd_fname=$_fwd_fname;
		$this->fwd_lname=$_fwd_lname;
		$this->desc=$_desc;
		$this->suburb=$_suburb;
		$this->tel=$_tel;
		$this->email=$_email;
	...
	public function SelectFromDB($_fwd_id)
	{
		global $db;
		if (!ereg("^([0-9]+)$",$_fwd_id))
		{
			$this->Errors[]=_lang(invalid_fwd_id);
			return false;
		}
		$this->fwd_id=$_fwd_id;
		$sql = 'SELECT * FROM `f4wd` WHERE `fwd_id` = '.$_fwd_id;
		...
		{
			$this->Errors[]=_lang(no_f4wd_found);
			return false;
		}

		$row = $db->sql_fetchrow($result);
		$this->fwd_fname=$row['fwd_fname'];
		$this->fwd_lname=$row['fwd_lname'];
		$this->desc=$row['desc'];
	...
	public function Insert($admin=false)
	{
		...
		$sql = 'INSERT INTO `f4wd` (`fwd_fname`, `fwd_lname`, `desc`, `suburb`, `tel`, `email`, `tag5`, `tag6`, `added_date`, `dont_send_mail`,`dont_send_sms`,`inital_dont_send_mail`, `yard`, `home_phone`, `postcode`,  `tag4`) 
		VALUES (\''.PreSql($this->fwd_fname).'\',  \''.PreSql($this->fwd_lname).'\',  \''.PreSql($this->desc).'\',  \''.PreSql($this->suburb).'\',  \''.PreSql($this->tel).'\',  \''.PreSql($this->email).'\',  \''.PreSql($this->tag5).'\',  \''.PreSql($this->tag6).'\',  \''.date('Y-m-d').'\',  \''.PreSql($this->dont_send_mail).'\', \''.PreSql($this->dont_send_mail).'\',  \''.PreSql($this->dont_send_mail).'\',  \''.$this->yard.'\',  \''.PreSql($this->home_phone).'\',  \''.PreSql($this->postcode).'\',  \''.PreSql($this->tag4).'\')';
		if(!$db->sql_query($sql))
		{
			$this->Errors[]=$db->sql_error_msg($result);
			return false;
		}
		$this->fwd_id=$db->sql_nextid("subscriber_id");
		return $this->fwd_id;
	}
	...
	public function Add($admin=false)
	{
		global $predir;
		$op='Add';
		include $predir.'forms/ff4wd.php';
	...
	public function Delete()
	{
		...
		$sql = 'DELETE FROM `f4wd` WHERE `fwd_id`='.$this->fwd_id;
		if(!$db->sql_query($sql))
		{
			$this->Errors[]=$db->sql_error_msg($result);
			return false;
	...
	{
		$fwd_id=PreForm($this->fwd_id);
		$fwd_fname=PreForm($this->fwd_fname);
		$fwd_lname=PreForm($this->fwd_lname);
	...
	{
		global $db;
		$sql = 'UPDATE `f4wd` SET `fwd_fname` = \''.PreSql($this->fwd_fname).'\', `fwd_lname` = \''.PreSql($this->fwd_lname).'\', `desc` = \''.PreSql($this->desc).'\', `suburb` = \''.PreSql($this->suburb).'\', `tel` = \''.PreSql($this->tel).'\', `email` = \''.PreSql($this->email).'\', `tag5` = \''.PreSql($this->tag5).'\', `tag6` = \''.PreSql($this->tag6).'\', `dont_send_mail` = \''.PreSql($this->dont_send_mail).'\', `dont_send_sms` = \''.PreSql($this->dont_send_sms).'\', `yard` = \''.PreSql($this->yard).'\', `home_phone` = \''.PreSql($this->home_phone).'\', `postcode` = \''.PreSql($this->postcode).'\', `tag4` = \''.PreSql($this->tag4).'\' WHERE `fwd_id` = '.$this->fwd_id;
	...
	public function SendEnquiryMail()
	{
		...
		$msg .= _lang("enquery_id").": FWD$this->fwd_id\n";
		//$msg .= _lang("enquery_date").": $this->added_date\n";
		$msg .= "First name: $this->fwd_fname\n";
		$msg .= "Surname: $this->fwd_lname\n";
		...
		MyMail($admin_mails.",info@austfleetsales.com.au",_lang(f4wd_mail_subject),nl2br($msg), "From: $this->email\nReply-To: $this->email\nContent-Type: text/html",_lang(f4wd_mail_subject));
	}

	public function FromForm()
	{
		$this->fwd_id=PostForm($_POST['fwd_id']);
		$this->fwd_fname=PostForm($_POST['fwd_fname'],50);
		$this->fwd_lname=PostForm($_POST['fwd_lname'],50);
	...
	public static function AdminListf4wds()
	{
		...
		$WHERE.=" AND lower(`fwd_fname`) LIKE lower('".$_GET['q_fname']."%') ";

		if(isset($_GET['q_lname'])&&$_GET['q_lname']!="")
		$WHERE.=" AND lower(`fwd_lname`) LIKE lower('".$_GET['q_lname']."%') ";

		if(isset($_GET['q_email'])&&$_GET['q_email']!="")
		$WHERE.=" AND lower(`email`) LIKE lower('".$_GET['q_email']."%') ";

		$sql = 'SELECT * FROM `f4wd` WHERE true '.$WHERE.'	ORDER BY `fwd_id` DESC LIMIT '.(($admin_page-1)*$per_page).','.($per_page+1).' ';

		$export_sql ='SELECT `fwd_id` as `'._lang(enquery_id).'` , `added_date` as `'._lang(enquery_date).'` ,  `fwd_fname` as `First Name` ,`fwd_lname` as `Surame`, `suburb` as `Suburb`, `email` as `Email Address`, `tel` as `Mobile`, `desc` as `Trade-In Description`,CASE WHEN `dont_send_mail` THEN \'no\' ELSE \'yes\' END as `'._lang(receive_mail).'` FROM `f4wd` WHERE true '.$WHERE.'	ORDER BY `fwd_id` DESC ';
		...
		$List= '<h3 class="admin_title">'._lang(list_f4wd).':</h3>' ;
		...
		while(($row = $db->sql_fetchrow($result))&&$i<$per_page)
		{
			...
			$List.= '<tr class="'.$class.'"><td width="10%">'.$row[fwd_id].'</td><td width="20%">'.
			"<a href=\"?fwd_id=$row[fwd_id]&op=Edit\">$row[fwd_fname]</a></td>".
			"<td width=\"20%\"><a href=\"?fwd_id=$row[fwd_id]&op=Edit\">$row[fwd_lname]</a></td>".
			"<td width=\"30%\"><a href=\"?fwd_id=$row[fwd_id]&op=Edit\">$row[email]</a></td>".
			"<td width=\"20%\"><a href=\"javascript:if (confirm('"._lang(sure_delete_f4wd)."')) {document.location ='?fwd_id=$row[fwd_id]&op=Delete';}\">Delete</a></td></tr> ";
		}
		$List.= '</table></form>';
		...
		<label for="job_id">&nbsp;&nbsp;<b>'._lang(enter_fwd_id).':</b></label>
		<input type="hidden" name="op" value="Edit" />
		<input type="text" name="fwd_id" />
		<input type="button" onClick="document.ProdForm.op.value=\'Edit\';document.ProdForm.submit();"  value="Edit" />
		<input onclick="if (confirm(\''._lang(sure_delete_f4wd).'\')) {document.ProdForm.op.value=\'Delete\';document.ProdForm.submit();} " value="Delete" type="button" />
	</form>
	<br/>';
	...
	public function SetAutoResponses()
	{
		...
		for ($i=2;$i<$emails_count;$i++)
		{
			...
			if(!$auto_mail->NewAutoEmail($i,$email_autoresponse_time[$i],$this->email,$this->fwd_fname,'f4wd'.$this->fwd_id))
			{
