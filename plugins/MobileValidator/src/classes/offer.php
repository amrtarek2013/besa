...
class offer
{
	...
	public function SetValues($_offer_id , $_offer_fname , $_offer_lname , $_desc , $_suburb , $_tel , $_email , $_yard , $_stock_no , $_added_date , $_dont_send_mail , $_address , $_home_phone , $_postcode , $_tag4)
	{
		$this->offer_fname=$_offer_fname;
		$this->offer_fname=$_offer_fname;
		$this->offer_lname=$_offer_lname;
		$this->desc=$_desc;
		$this->suburb=$_suburb;
		$this->tel=$_tel;
		$this->email=$_email;
	...
	public function SelectFromDB($_offer_id)
	{
		global $db;
		if (!ereg("^([0-9]+)$",$_offer_id))
		{
			$this->Errors[]=_lang(invalid_offer_id);
			return false;
		}
		$this->offer_id=$_offer_id;
		$sql = 'SELECT * FROM `offer` WHERE `offer_id` = '.$_offer_id;
		...
		{
			$this->Errors[]=_lang(no_offer_found);
			return false;
		}

		$row = $db->sql_fetchrow($result);
		$this->offer_fname=$row['offer_fname'];
		$this->offer_lname=$row['offer_lname'];
		$this->desc=$row['desc'];
	...
	public function Insert($admin=false)
	{
		...
		$sql = 'INSERT INTO `offer` (`offer_fname`, `offer_lname`, `desc`, `suburb`, `tel`, `email`, `yard`, `stock_no`, `added_date`, `dont_send_mail`, `dont_send_sms`, `inital_dont_send_mail`, `address`, `home_phone`, `postcode`,  `tag4`) VALUES (\''.PreSql($this->offer_fname).'\',  \''.PreSql($this->offer_lname).'\',  \''.PreSql($this->desc).'\',  \''.PreSql($this->suburb).'\',  \''.PreSql($this->tel).'\',  \''.PreSql($this->email).'\',  \''.PreSql($this->yard).'\',  \''.PreSql($this->stock_no).'\',  \''.date('Y-m-d').'\',  \''.PreSql($this->dont_send_mail).'\',  \''.PreSql($this->dont_send_mail).'\', \''.PreSql($this->dont_send_mail).'\',  \''.PreSql($this->address).'\',  \''.PreSql($this->home_phone).'\',  \''.PreSql($this->postcode).'\',  \''.PreSql($this->tag4).'\')';
		if(!$db->sql_query($sql))
		{
			$this->Errors[]=$db->sql_error_msg($result);
			return false;
		...
		$this->offer_id=$db->sql_nextid("subscriber_id");
	...
	public function Add($stock_no,$admin=false)
	{
		global $predir;
		$op='Add';
		include $predir.'forms/foffer.php';
	...
	public function Delete()
	{
		...
		$sql = 'DELETE FROM `offer` WHERE `offer_id`='.$this->offer_id;
		if(!$db->sql_query($sql))
		{
			$this->Errors[]=$db->sql_error_msg($result);
			return false;
	...
	{
		$offer_id=PreForm($this->offer_id);
		$offer_fname=PreForm($this->offer_fname);
		$offer_lname=PreForm($this->offer_lname);
	...
	{
		global $db;
		$sql = 'UPDATE `offer` SET `offer_fname` = \''.PreSql($this->offer_fname).'\', `offer_lname` = \''.PreSql($this->offer_lname).'\', `desc` = \''.PreSql($this->desc).'\', `suburb` = \''.PreSql($this->suburb).'\', `tel` = \''.PreSql($this->tel).'\', `email` = \''.PreSql($this->email).'\', `yard` = \''.PreSql($this->yard).'\', `stock_no` = \''.PreSql($this->stock_no).'\', `dont_send_mail` = \''.PreSql($this->dont_send_mail).'\', `dont_send_sms` = \''.PreSql($this->dont_send_sms).'\', `address` = \''.PreSql($this->address).'\', `home_phone` = \''.PreSql($this->home_phone).'\', `postcode` = \''.PreSql($this->postcode).'\', `tag4` = \''.PreSql($this->tag4).'\' WHERE `offer_id` = '.$this->offer_id;
	...
	{
		$from="info@austfleetoffers.com.au";
		$msg .= _lang("enquery_id").": OFR$this->offer_id\n\n";
	...
	public function SendEnquiryMail()
	{
		...
		$msg .= "First name: $this->offer_fname\n";
		$msg .= "Last Name: $this->offer_lname\n";
		...
		MyMail($admin_mails,_lang(offer_mail_subject),nl2br($msg), "From: $this->email\nReply-To: $this->email\nContent-Type: text/html",_lang(offer_mail_subject));
	}

	public function FromForm()
	{
		$this->offer_id=PostForm($_POST['offer_id']);
		$this->offer_fname=PostForm($_POST['offer_fname'],50);
		$this->offer_lname=PostForm($_POST['offer_lname'],50);
	...
	public static function AdminListoffers()
	{
		...
		$WHERE.=" AND lower(`offer_fname`) LIKE lower('".$_GET['q_fname']."%') ";

		if(isset($_GET['q_lname'])&&$_GET['q_lname']!="")
		$WHERE.=" AND lower(`offer_lname`) LIKE lower('".$_GET['q_lname']."%') ";

		if(isset($_GET['q_email'])&&$_GET['q_email']!="")
		$WHERE.=" AND lower(`email`) LIKE lower('".$_GET['q_email']."%') ";

		$sql = 'SELECT * FROM `offer` WHERE true '.$WHERE.'	ORDER BY `offer_id` DESC LIMIT '.(($admin_page-1)*$per_page).','.($per_page+1).' ';

		$export_sql ='SELECT `offer_id` as `'._lang(enquery_id).'` , `added_date` as `'._lang(enquery_date).'` ,  `offer_fname` as `First Name` ,`offer_lname` as `Surame`, `suburb` as `Suburb`, `email` as `Email Address`, `tel` as `Mobile`, `desc` as `Trade-In Description`,CASE WHEN `dont_send_mail` THEN \'no\' ELSE \'yes\' END as `'._lang(receive_mail).'` FROM `offer` WHERE true '.$WHERE.'	ORDER BY `offer_id` DESC ';
		...
		$List= '<h3 class="admin_title">'._lang(list_offer).':</h3>' ;
		...
		while(($row = $db->sql_fetchrow($result))&&$i<$per_page)
		{
			...
			$List.= '<tr class="'.$class.'"><td width="10%">'.$row[offer_id].'</td><td width="20%">'.
			"<a href=\"?offer_id=$row[offer_id]&op=Edit\">$row[offer_fname]</a></td>".
			"<td width=\"20%\"><a href=\"?offer_id=$row[offer_id]&op=Edit\">$row[offer_lname]</a></td>".
			"<td width=\"30%\"><a href=\"?offer_id=$row[offer_id]&op=Edit\">$row[email]</a></td>".
			"<td width=\"20%\"><a href=\"javascript:if (confirm('"._lang(sure_delete_offer)."')) {document.location ='?offer_id=$row[offer_id]&op=Delete';}\">Delete</a></td></tr> ";
		}
		$List.= '</table></form>';
		...
		<label for="job_id">&nbsp;&nbsp;<b>'._lang(enter_offer_id).':</b></label>
		...
		<input onclick="if (confirm(\''._lang(sure_delete_offer).'\')) {document.ProdForm.op.value=\'Delete\';document.ProdForm.submit();} " value="Delete" type="button" />
	</form>
	<br/>';
		...
		public function SetAutoResponses()
	{
		...
		{
			if(!$auto_mail->NewAutoEmail($i,$email_autoresponse_time[$i],$this->email,$this->offer_fname,'offer'.$this->offer_id))
			{
