...
// $internalTimeStr  = '2000-05-01 22:30:20';  // from database / system
// // and this is the conversion
// $internalTime     = strtotime($internalTimeStr);
// $externalTime     = utc2local($internalTimeStr,$externalTimezone);
// $externalTimeStr  = date('Y-m-d H:i:s', $externalTime);
// echo $externalTimeStr;die;
?>     
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0"><?=__('Users During The Day')?></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?=ADMIN_LINK?>"><?=__('Home')?></a></li>
                        <li class="breadcrumb-item active"><?=__('Users During The Day')?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="container-fluid">
                ...
                <div class="col-lg-12">
                    ...
                    <?= $this->AdminForm->create($user, ['type' => 'file',"id"=>"filter_form"]) ?>
                        ...
                        <div class="card-body row">
                            ...
                            <div class="col-lg-2">
                            <div class="form-group password">
                                <label><?=__('Time Frame')?></label>
                                <select name="time_frame" class="form-control" id="time_frame">
                                    <?php foreach ($time_frame_list as $key => $value) { ?>
                                        <option value="<?=$key?>" <?=!empty($time_frame) && $time_frame==$key?'selected':''?> ><?=$value?></option>
                                    <?php } ?>
                                </select>
                            </div>                        
                            </div>
                            ...
                            <div class="form-group password">
                                ...
                                <select name="time_zone" class="form-control" id="time_zone">
                                    <?php foreach ($time_zone_list as $key => $value) { ?>
                                        <option value="<?=$key?>" <?=!empty($time_zone) && $time_zone==$key?'selected':''?> ><?=$value?></option>
                                    <?php } ?>
                                </select>
                            </div>                        
                            </div>
            ...
            <div class="row">
                    ...
                    <div class="card">
                        <div class="card-header border-0">
                            <div class="d-flex justify-content-between">
                                <h3 class="card-title card-title-report"><?=__('Users During The Day')?></h3>
                        ...
                        <div class="card-body">
                            <div class="d-flex">
                                ...
                                <p class="ml-auto d-flex flex-column text-right">
                                    <span class="text-success">
                                        <i class="fas fa-arrow-up"></i> <?=$total_users?>
                                    </span>
                            ...
                            <div class="d-flex flex-row justify-content-end">
                                <span class="mr-2">
                                    <i class="fas fa-square text-primary"></i> <?=__('Day Hours')?>
                                </span>
                                <!-- <span>
                ...
                <div class="col-lg-6">        
                    <div class="card">
                        <div class="card-header border-0">
                            <h3 class="card-title card-title-report"><?=__('Users During The Day Report')?></h3>
                            <div class="card-tools">
                                <a href="#" class="btn btn-tool btn-sm export_btn" ><i class="fas fa-download"></i></a>
                            </div>
                        </div>
                        <div class="card-body table-responsive p-0">
                            <table class="table table-striped table-valign-middle">
                                ...
                                <tbody>
                                    <?php foreach ($users_per_hour as $key => $value) { 
                                        if(!empty($time_zone) && in_array($time_zone, ["2","3"])){
                                            if( !in_array($time_frame, ["15","30","45"]) ){
                                                $cur_h_n  = $value->cdate*intval($time_frame);
                                                
                                                // $cur_h = sprintf("%02d", $cur_h_n).":00"; 
                                                $cur_h1 = sprintf("%02d", $cur_h_n).":00"; 
                                                $cur_h = date('H:i',strtotime('+'.$time_zone.' hour',strtotime($cur_h1)));
                                                $time_frame_minutes = 60*intval($time_frame);
                                            }else{
                                                $cur_h  = $value->cdate;
                                                $time_frame_minutes = intval($time_frame);
                                            }
                                        }else{
                                            if( !in_array($time_frame, ["15","30","45"]) ){
                                                $cur_h_n  = $value->cdate*intval($time_frame);
                                        ...
                                        // $cur_h  = date("H:i", mktime($value->cdate, 0, 0, 0, 0, 0));
                                        $cur_h_1= date('H:i',strtotime($cur_h)+$time_frame_minutes*60);
                                        ?>
                                        <tr>
                                            <td><?=$cur_h."-".$cur_h_1?></td>
