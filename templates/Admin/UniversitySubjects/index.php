...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><?= __($mainUniversityTitle . ' Subjects List') ?></h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?= ADMIN_LINK ?>"><?= __('Home') ?></a></li>
                        <li class="breadcrumb-item active"><?= __($mainUniversityTitle . ' Subjects') ?></li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        echo $this->List->filter_form($universitySubjects, $filters, [], [], $parameters, $session) ?>
                    </div>
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><?= __($mainUniversityTitle . ' Subjects List') ?></h3>
                            <a class="add-new-btn btn btn-primary <?= $currLang == 'en' ? 'float-right' : 'float-left' ?>" href="<?= ADMIN_LINK ?>/universitySubjects/add">
                                <?= __('Add new') ?>
                            </a>
                        </div>
                        <?php $fields = [
                            'basicModel' => 'universitySubjects',
                            'id' => [],
                            'university.university_name' => ['title' => 'University'],
                            'subject_area.title' => ['title' => 'Subject Area'],
                            'rank' => [],
                            'apply_rank' => [],
                            // 'active' => ['format' => 'bool'],
                        ];
                        ...
                        echo $this->List->adminIndex($fields, $universitySubjects, $actions, false, $multi_select_actions, $parameters);
                        ?>
                    </div>
                </div>
            </div>
